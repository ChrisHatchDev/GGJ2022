...
//Global variables
fixed4 _SeerPosition[64];
float _SeerRadius[64];
float _SeerGradient[64];
fixed4 _SeerColor[64];
int _SeerShape[64];
float3 _SeerSize[64];
float _SeerHider[64];
int _Seers;
...
///-------------------------
fixed4 Calculate_Reveal(fixed4 Albedo, fixed4 WorldPosition, float Hidable)
{
	//Close enough to at least one Seer ?	
	bool Visible = false;
	float SavedVisibility = 1.0f;
	fixed4 SavedColorGradient;
	for (int Indice = 0; Indice < _Seers; Indice++)
	{
		...
		fixed4 SeerColor = _SeerColor[Indice];
		...
		{
			//Simple sphere
			Distance = distance(SeerPosition, WorldPosition) * _Resistance;
		}
